...
<div class="modal-bx">
  ...
  <form id="form-plano">
    ...
    <div class="row2">
      ...
      <div class="fg"><label>Periodicidade *</label>
        <select name="periodicidade" required>
          <option value="">Selecione…</option>
          ${E.map(r=>`<option value="${r}"${(t==null?void 0:t.periodicidade)===r?" selected":""}>${r}</option>`).join("")}
        </select></div>
    </div>
...
</div>`,document.body.appendChild(a),a.querySelector("#btn-cancel").addEventListener("click",()=>a.remove()),a.addEventListener("click",r=>{r.target===a&&a.remove()}),a.querySelector("#form-plano").addEventListener("submit",async r=>{r.preventDefault();const i=new FormData(r.target),e={nome:i.get("nome").trim(),negocio:i.get("negocio"),valor:parseFloat(i.get("valor"))||0,periodicidade:i.get("periodicidade"),status:i.get("status")||"Ativo",descricao:i.get("descricao").trim()};if(!e.negocio||!e.periodicidade){c("Preencha todos os campos obrigatórios.","aviso");return}const n=r.target.querySelector("[type=submit]");n.disabled=!0,n.textContent="Salvando…";try{await $({...t||{},...e}),c(o?"Plano atualizado!":"Plano criado!","ok"),a.remove(),s=await g(),p()}catch(h){c("Erro: "+h.message,"erro"),n.disabled=!1,n.textContent=o?"Salvar":"Criar plano"}})}async function A(t,o){const a=s.find(i=>i.id===t);if(!a)return;const r=o==="Ativo"?"Inativo":"Ativo";if(confirm(`Deseja ${r==="Inativo"?"inativar":"ativar"} o plano "${a.nome}"?`))try{await $({...a,status:r}),c(`Plano ${r==="Ativo"?"ativado":"inativado"}!`,"ok"),s=await g(),p()}catch(i){c("Erro: "+i.message,"erro")}}async function P(t){const o=s.find(a=>a.id===t);if(o&&confirm(`Excluir permanentemente o plano "${o.nome}"?

Esta ação não pode ser desfeita.`))try{await S(t),c("Plano excluído!","ok"),s=await g(),p()}catch(a){c("Erro: "+a.message,"erro")}}async function C(t){l=t,b="",u="",v="",t.innerHTML='<div style="padding:2rem;text-align:center;color:var(--text-muted)">Carregando planos…</div>',s=await g(),p()}export{C as initPlanos};
