import{i as J,s as v,u as G,j as Q,t as h,f as $,b as _,e as F}from"./page-dashboard-Dbqm2OjXb.js";import{u as B}from"./page-lancamentos-BfWtlLw7b.js";import"./supabase-DthfXWp1.js";let L=null,E="lista",N="",S="todos",P="",selCli=new Set();const U={provedor:{vencidoDias:1,bloqueadoDias:6,cobrancasInativo:null},servidor:{vencidoDias:1,bloqueadoDias:2,cobrancasInativo:3},default:{vencidoDias:1,bloqueadoDias:6,cobrancasInativo:null}},tt=(t="")=>U[(t||"").toLowerCase()]||U.default,X=["Provedor","Servidor","Precautec","Outro"],et=["Mensal","Bimestral","Trimestral","Semestral","Anual"];function O(t){if(!t)return 0;const d=new Date;return d.setHours(0,0,0,0),Math.floor((d-new Date(t+"T00:00:00"))/864e5)}function k(t,d){const e=(d==null?void 0:d.get(t.id))||[];if(!e.length)return(t.contratos||[]).length?"ativo":"sem-contrato";let a=-1/0,r="";for(const p of e){const f=O(p.data);f>a&&(a=f,r=p.negocio||"")}if(a<=0)return"ativo";const i=tt(r);if(i.cobrancasInativo){const p=(t.contratos||[]).find(f=>{var n;return((n=f.negocio)==null?void 0:n.toLowerCase())===r.toLowerCase()});if(p&&(p.cobrancasEnviadas||0)>=i.cobrancasInativo)return"inativo"}return a>=i.bloqueadoDias?"bloqueado":a>=i.vencidoDias?"vencido":"ativo"}const j={ativo:{label:"Ativo",bg:"#d1fae5",cor:"#065f46"},vencido:{label:"Vencido",bg:"#fef3c7",cor:"#92400e"},bloqueado:{label:"Bloqueado",bg:"#fee2e2",cor:"#991b1b"},inativo:{label:"Inativo",bg:"#f3f4f6",cor:"#6b7280"},"sem-contrato":{label:"Sem contrato",bg:"#eff6ff",cor:"#1d4ed8"},novo:{label:"Novos do mês",bg:"#dbeafe",cor:"#1e40af"}};async function R(){try{const[t,d]=await Promise.all([J("cli_clientes"),J("cli_planos")]);v.set("clientes",G("cli_clientes",t)),v.set("cli_planos",G("cli_planos",d));const{data:e}=await Q.from("lancamentos").select("id, dados").eq("dados->>tipo","receita").neq("dados->>status","pago"),a=new Map;for(const r of e||[]){const i=r.dados||{};if(!i.clienteId)continue;const p=a.get(i.clienteId)||[];p.push({id:r.id,...i}),a.set(i.clienteId,p)}v.set("faturas_abertas_map",a)}catch(t){h.err("Erro ao carregar: "+t.message)}Z()}function Z(){if(!L)return;const d=`
    ...
    <div id="cli-bulk-bar" style="display:flex;align-items:center;gap:10px;padding:6px 2px;margin-bottom:4px;flex-wrap:wrap;"><label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;color:var(--text-2,#6b7280);"><input type="checkbox" id="chk-all-cli" ${e.length>0&&e.every(_n=>selCli.has(_n.id))?"checked":""} style="width:15px;height:15px;accent-color:var(--accent,#3FD68E);" /> Selecionar todos (${e.length})</label>${selCli.size>0?`<button id="btn-del-sel-cli" style="padding:5px 14px;border-radius:8px;border:none;cursor:pointer;background:#fee2e2;color:#991b1b;font-size:13px;font-weight:600;margin-left:auto;">🗑️ Excluir (${selCli.size})</button>`:""}</div><div id="lista-cli" style="display:flex;flex-direction:column;gap:6px;">
  ...
  `,(i=t.querySelector("#btn-novo-cli"))==null||i.addEventListener("click",()=>V()),(p=t.querySelector("#busca-cli"))==null||p.addEventListener("input",n=>{N=n.target.value;const _gid=n.target.id;clearTimeout(window._gtBT);window._gtBT=setTimeout(()=>{T(t);const _b=_gid&&document.getElementById(_gid);if(_b){_b.focus();_b.setSelectionRange(_b.value.length,_b.value.length)}},350)}),(f=t.querySelector("#sel-neg"))==null||f.addEventListener("change",n=>{P=n.target.value;selCli.clear();T(t)}),t.querySelectorAll("[data-fs]").forEach(n=>n.addEventListener("click",()=>{S=n.dataset.fs;selCli.clear();T(t)})),t.querySelector("#chk-all-cli")?.addEventListener("change",_ce=>{if(_ce.target.checked)e.forEach(_cc=>selCli.add(_cc.id));else e.forEach(_cc=>selCli.delete(_cc.id));T(t)});t.querySelectorAll("[data-chk-cli]").forEach(_cn=>_cn.addEventListener("change",_cs=>{_cs.target.checked?selCli.add(_cn.dataset.chkCli):selCli.delete(_cn.dataset.chkCli);const _bar=t.querySelector("#cli-bulk-bar");if(_bar){const _b=t.querySelector("#btn-del-sel-cli");if(selCli.size>0){if(!_b){const _nb=document.createElement("button");_nb.id="btn-del-sel-cli";_nb.style="padding:5px 14px;border-radius:8px;border:none;cursor:pointer;background:#fee2e2;color:#991b1b;font-size:13px;font-weight:600;margin-left:auto;";_nb.textContent="🗑️ Excluir ("+selCli.size+")";_bar.appendChild(_nb);_nb.addEventListener("click",_bdc)}}else{_b&&_b.remove()}if(_b)_b.textContent="🗑️ Excluir ("+selCli.size+")"}}));const _bdc=async()=>{if(!selCli.size)return;if(!confirm("Excluir permanentemente "+selCli.size+" cliente(s)?\n\nEsta ação não pode ser desfeita."))return;const _dids=[...selCli];selCli.clear();let _dok=0,_derr=0;for(const _did of _dids){try{const{error:_er}=await Q.from("cli_clientes").delete().eq("id",_did);if(_er)throw _er;_dok++}catch(_ex){_derr++;console.error(_ex)}}h.ok(_dok+" cliente(s) excluído(s)"+(_derr?" ("+_derr+" erro(s))":".")+"");await R()};t.querySelector("#btn-del-sel-cli")?.addEventListener("click",_bdc);t.querySelectorAll("[data-ver]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation();const o=(v.get("clientes")??[]).find(l=>l.id===n.dataset.ver);o&&rt(o,d)})),t.querySelectorAll("[data-edit]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation();const o=(v.get("clientes")??[]).find(l=>l.id===n.dataset.edit);o&&V(o)})),t.querySelectorAll("[data-cobrar]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation(),H(n.dataset.cobrar)})),t.querySelectorAll("[data-bloquear]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation(),Y(n.dataset.bloquear,!0)})),t.querySelectorAll("[data-reativar]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation(),Y(n.dataset.reativar,!1)})),t.querySelectorAll("[data-excluir]").forEach(n=>n.addEventListener("click",async s=>{s.stopPropagation();const o=(v.get("clientes")??[]).find(l=>l.id===n.dataset.excluir);if(!o)return;if(!confirm("Excluir permanentemente \""+(o.nome||o.id)+"\""+"?\n\nEsta ação não pode ser desfeita."))return;try{const{error:_er}=await Q.from("cli_clientes").delete().eq("id",o.id);if(_er)throw _er;h.ok("Cliente excluído.");await R()}catch(_er){h.err("Erro ao excluir: "+_er.message)}}))}function ot(t){return`<p style="text-align:center;padding:48px 0;color:#9ca3af;font-size:14px;">${t?"Nenhum cliente encontrado.":"Nenhum cliente cadastrado ainda."}</p>`}function at(t,d){var o;const e=k(t,d),a=j[e]||j.ativo,r=[...new Set((t.contratos||[]).map(l=>l.negocio).filter(Boolean))],i=(t.contratos||[]).filter(l=>l.statusContrato==="Ativo").reduce((l,x)=>l+(Number(x.valor)||0),0),p=e==="bloqueado"||e==="inativo",n=((d==null?void 0:d.get(t.id))||[]).filter(l=>O(l.data)<0).sort((l,x)=>{var c;return(c=l.data)==null?void 0:c.localeCompare(x.data)}),s=(o=n[0])!=null&&o.data?_(n[0].data):"—";return`
    <div style="background:var(--bg-card,#fff);border-radius:10px;padding:12px 14px;display:flex;align-items:center;gap:12px;box-shadow:0 1px 4px rgba(0,0,0,.06);border-left:3px solid ${a.bg};">
      <input type="checkbox" data-chk-cli="${t.id}" ${selCli.has(t.id)?"checked":""} style="width:16px;height:16px;cursor:pointer;flex-shrink:0;accent-color:var(--accent,#3FD68E);" onclick="event.stopPropagation()" /><div style="width:38px;height:38px;border-radius:50%;background:${a.bg};display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0;">👤</div>
