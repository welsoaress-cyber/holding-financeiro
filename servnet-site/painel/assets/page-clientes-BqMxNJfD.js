  ...
  `,(i=t.querySelector("#btn-novo-cli"))==null||i.addEventListener("click",()=>V()),(p=t.querySelector("#busca-cli"))==null||p.addEventListener("input",n=>{N=n.target.value;const _gid=n.target.id;clearTimeout(window._gtBT);window._gtBT=setTimeout(()=>{T(t);const _b=_gid&&document.getElementById(_gid);if(_b){_b.focus();_b.setSelectionRange(_b.value.length,_b.value.length)}},350)}),(f=t.querySelector("#sel-neg"))==null||f.addEventListener("change",n=>{P=n.target.value,T(t)}),t.querySelectorAll("[data-fs]").forEach(n=>n.addEventListener("click",()=>{S=n.dataset.fs,T(t)})),t.querySelectorAll("[data-ver]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation();const o=(v.get("clientes")??[]).find(l=>l.id===n.dataset.ver);o&&rt(o,d)})),t.querySelectorAll("[data-edit]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation();const o=(v.get("clientes")??[]).find(l=>l.id===n.dataset.edit);o&&V(o)})),t.querySelectorAll("[data-cobrar]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation(),H(n.dataset.cobrar)})),t.querySelectorAll("[data-bloquear]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation(),Y(n.dataset.bloquear,!0)})),t.querySelectorAll("[data-reativar]").forEach(n=>n.addEventListener("click",s=>{s.stopPropagation(),Y(n.dataset.reativar,!1)}))}function ot(t){return`<p style="text-align:center;padding:48px 0;color:#9ca3af;font-size:14px;">${t?"Nenhum cliente encontrado.":"Nenhum cliente cadastrado ainda."}</p>`}function at(t,d){var o;const e=k(t,d),a=j[e]||j.ativo,r=[...new Set((t.contratos||[]).map(l=>l.negocio).filter(Boolean))],i=(t.contratos||[]).filter(l=>l.statusContrato==="Ativo").reduce((l,x)=>l+(Number(x.valor)||0),0),p=e==="bloqueado"||e==="inativo",n=((d==null?void 0:d.get(t.id))||[]).filter(l=>O(l.data)<0).sort((l,x)=>{var c;return(c=l.data)==null?void 0:c.localeCompare(x.data)}),s=(o=n[0])!=null&&o.data?_(n[0].data):"—";return`
    <div style="background:var(--bg-card,#fff);border-radius:10px;padding:12px 14px;display:flex;align-items:center;gap:12px;box-shadow:0 1px 4px rgba(0,0,0,.06);border-left:3px solid ${a.bg};">
      ...
      <div style="flex:1;min-width:0;">
        <div style="font-weight:600;font-size:14px;color:var(--text-1,#111);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${t.codigo?`<span style="font-size:10px;font-weight:700;color:var(--text-2,#9ca3af);margin-right:5px;">#${t.codigo}</span>`:""} ${z(t.nome||"—")}</div>
...
`),p=document.createElement("a");p.href="data:text/csv;charset=utf-8,\uFEFF"+encodeURIComponent(i),p.download=`${d}-${new Date().toISOString().slice(0,10)}.csv`,p.click(),h.ok(`CSV exportado: ${t.length} linha(s)`)}function rt(t,d){var o,l,x;const e=k(t,d),a=j[e],r=(d==null?void 0:d.get(t.id))||[],i=(t.historicoCobrancas||[]).slice(-20).reverse(),p=(t.contratos||[]).map(c=>`
    <tr>
      <td style="padding:8px 6px;"><span style="font-size:11px;font-weight:700;background:var(--bg,#f3f4f6);padding:2px 6px;border-radius:6px;color:var(--text-2,#374151);">${t.codigo||"?"}/${c.codigoContrato||"?"}</span><div style="font-size:12px;color:var(--text-2,#6b7280);margin-top:2px;">${c.negocio||"—"}</div></td>
      ...
      <td style="padding:8px 6px;">${(()=>{const _b=c.beneficios||{};const _keys=["va","vr","vt","eq","ot"];const _divg=_keys.some(k=>Math.abs(((_b[k]||{}).planilha||0)-((_b[k]||{}).sistema||0))>0.01);const _tot=_keys.reduce((s,k)=>((_b[k]||{}).planilha||0)+s,0);return _tot>0?`${_divg?"⚠️":"✅"} ${$(_tot)}`:_divg?"⚠️":"—"})()}</td>
      <td style="padding:8px 6px;"><button data-edit-ct="${c.id}" style="padding:3px 8px;border-radius:6px;border:none;cursor:pointer;background:#fef9c3;color:#92400e;font-size:12px;">✏️</button></td>
    </tr>`).join(""),f=i.map(c=>`
    <tr style="border-bottom:1px solid var(--border,#f3f4f6);">
      ...
      <div style="margin-bottom:20px;">
        ...
        ${(t.contratos||[]).length?`<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:13px;">
              <thead><tr style="color:var(--text-2,#6b7280);font-size:11px;text-transform:uppercase;">
                <th style="padding:6px;text-align:left;">Código/Negócio</th><th style="padding:6px;text-align:left;">Plano</th>
                <th style="padding:6px;text-align:left;">Valor</th><th style="padding:6px;text-align:left;">Vencimento</th>
                <th style="padding:6px;text-align:left;">Periodicidade</th><th style="padding:6px;text-align:left;">Status</th><th style="padding:6px;text-align:left;">Benefícios</th><th style="padding:6px;"></th>
              </tr></thead>
  ...
  `,s=W(`Cliente — ${z(t.nome||"")}`,n);(o=s.querySelector("#btn-add-ct"))==null||o.addEventListener("click",()=>{s.remove(),st(t,null)}),(l=s.querySelector("#btn-det-edit"))==null||l.addEventListener("click",()=>{s.remove(),V(t)}),(x=s.querySelector("#btn-det-cobrar"))==null||x.addEventListener("click",()=>{s.remove(),H(t.id)}),s.querySelectorAll("[data-edit-ct]").forEach(b=>{b.addEventListener("click",()=>{const c=(t.contratos||[]).find(z=>z.id===b.dataset.editCt);c&&(s.remove(),st(t,c))})})}function V(t=null){var i,p;const d=!t,e=t||{},a=`
    <form id="form-cli" style="display:flex;flex-direction:column;gap:12px;font-family:Inter,sans-serif;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        ...
        ${u("codigo","Código do cliente","text",e.codigo||"")}
    ...
    </form>`,r=W(d?"Novo Cliente":"Editar Cliente",a);(i=r.querySelector("#btn-cancel-cli"))==null||i.addEventListener("click",()=>r.remove()),(p=r.querySelector("#form-cli"))==null||p.addEventListener("submit",async f=>{f.preventDefault();const n=new FormData(f.target),s=Object.fromEntries(n.entries()),o={...e||{},nome:e.nome===s.nome?e.nome:s.nome,...s,contratos:e.contratos||[],historicoCobrancas:e.historicoCobrancas||[],dataCadastro:e.dataCadastro||new Date().toISOString().split("T")[0]};try{await F("cli_clientes",{id:e.id||B(),dados:o}),h.ok(d?"Cliente criado!":"Cliente atualizado!"),r.remove(),await R()}catch(l){h.err("Erro: "+l.message)}})}function st(t,d=null){var f,n,s;const e=!d,a=d||{},r=(v.get("cli_planos")||[]).filter(o=>!o.status||o.status==="Ativo"),i=`
    <form id="form-ct" style="display:flex;flex-direction:column;gap:12px;font-family:Inter,sans-serif;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        ...
        ${u("codigoContrato","Código do contrato","text",a.codigoContrato||"")}
      </div>
      <details style="margin-top:4px;"><summary style="cursor:pointer;font-size:12px;font-weight:600;color:var(--text-2,#6b7280);padding:6px 0;">💰 Benefícios (Planilha vs Sistema)</summary>
      <div style="overflow-x:auto;margin-top:8px;"><table style="width:100%;border-collapse:collapse;font-size:12px;">
        <thead><tr style="color:var(--text-2,#6b7280);font-size:11px;"><th style="padding:4px 6px;text-align:left;">Benefício</th><th style="padding:4px 6px;">Planilha (R$)</th><th style="padding:4px 6px;">Sistema (R$)</th></tr></thead>
        <tbody>
          ${[["va","VA"],["vr","VR"],["vt","VT"],["eq","Equipamentos"],["ot","Outros"]].map(([k,lbl])=>`<tr><td style="padding:4px 6px;font-weight:600;">${lbl}</td><td style="padding:4px;"><input name="b${k}_p" type="number" step="0.01" value="${((a.beneficios||{})[k]||{}).planilha||0}" style="width:90px;padding:4px 6px;border-radius:6px;border:1px solid var(--border,#e5e7eb);font-size:12px;background:var(--bg,#fff);color:var(--text-1,#111);"/></td><td style="padding:4px;"><input name="b${k}_s" type="number" step="0.01" value="${((a.beneficios||{})[k]||{}).sistema||0}" style="width:90px;padding:4px 6px;border-radius:6px;border:1px solid var(--border,#e5e7eb);font-size:12px;background:var(--bg,#fff);color:var(--text-1,#111);"/></td></tr>`).join("")}
