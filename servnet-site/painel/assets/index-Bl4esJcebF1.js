    ...
    <tr style="border-bottom:1px solid var(--border,#f3f4f6);${e.dias>5?"background:rgba(254,226,226,.25);":""}">
      ...
      <td style="padding:10px 6px;text-align:center;">
        <div style="display:flex;gap:4px;justify-content:center;">
          ...
          <button data-editar="${e.id}" title="Editar fatura" style="${q("#e0f2fe","#0c4a6e")}">✎</button>
          <button data-bloquear-cli="${e.clienteId}" title="Bloquear" style="${q("#fee2e2","#991b1b")}">⊘</button>
        </div>
      </td>
    </tr>`}function q(e,a){return`padding:4px 8px;border-radius:6px;border:none;cursor:pointer;font-size:13px;background:${e};color:${a};`}async function N(e){if(e){c.ok("Enviando cobrança…");try{const{data:a,error:o}=await g.functions.invoke("whatsapp-lembretes",{body:{clienteId:e}});if(o||a!=null&&a.error)throw new Error((o==null?void 0:o.message)||(a==null?void 0:a.error));const i=(a==null?void 0:a.enviados)??0;c.ok(i>0?`✅ ${i} mensagem(ns) enviada(s)!`:"Nenhuma fatura encontrada para este cliente.")}catch(a){c.err("Erro: "+a.message)}}}async function T(){if(!p.size){c.err("Selecione ao menos uma fatura.");return}const e=u.get("faturas_cobr")||[],a=[...new Set([...p].map(i=>{var r;return(r=e.find(b=>b.id===i))==null?void 0:r.clienteId}).filter(Boolean))];c.ok(`Enviando cobrança para ${a.length} cliente(s)…`);let o=0;for(const i of a)try{const{data:r}=await g.functions.invoke("whatsapp-lembretes",{body:{clienteId:i}});o+=(r==null?void 0:r.enviados)||0}catch{}c.ok(`✅ ${o} mensagem(ns) enviada(s)!`),p.clear(),await w()}async function M(){if(confirm("Disparar lembretes automáticos para todos os clientes com faturas em atraso?")){c.ok("Disparando…");try{const{data:e,error:a}=await g.functions.invoke("whatsapp-lembretes");if(a||e!=null&&e.error)throw new Error((a==null?void 0:a.message)||(e==null?void 0:e.error));c.ok(`Enviados: ${(e==null?void 0:e.enviados)||0} | Erros: ${(e==null?void 0:e.erros)||0}`)}catch(e){c.err("Erro: "+e.message)}}}async function GT_editFat(id){try{const{data:o,error:er}=await g.from("lancamentos").select("dados").eq("id",id).single();if(er)throw er;const d=(o&&o.dados)||{};const esc=t=>String(t==null?"":t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/"/g,"&quot;");const st=(d.status||"Pendente").toLowerCase();const ov=document.createElement("div");ov.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center;z-index:2000;padding:16px;";ov.innerHTML=`<div style="background:#fff;color:#0f172a;border-radius:14px;max-width:420px;width:100%;padding:20px;font-family:Inter,sans-serif;box-shadow:0 12px 40px rgba(0,0,0,.25);">
<div style="font-weight:700;font-size:16px;margin-bottom:14px;">Editar fatura</div>
<label style="font-size:12px;color:#64748b;">Descrição</label>
<input id="gt-ef-desc" value="${esc(d.descricao)}" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:8px;margin:4px 0 10px;background:#fff;color:#0f172a;">
<label style="font-size:12px;color:#64748b;">Valor (R$)</label>
<input id="gt-ef-valor" type="number" step="0.01" value="${esc(d.valor)}" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:8px;margin:4px 0 10px;background:#fff;color:#0f172a;">
<label style="font-size:12px;color:#64748b;">Vencimento</label>
<input id="gt-ef-venc" type="date" value="${esc((d.data_vencimento||d.data||"").slice(0,10))}" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:8px;margin:4px 0 10px;background:#fff;color:#0f172a;">
<label style="font-size:12px;color:#64748b;">Negócio</label>
<select id="gt-ef-neg" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:8px;margin:4px 0 10px;background:#fff;color:#0f172a;">${(()=>{const atual=d.negocio||"";const lst=[...new Set([atual,...(u.get("negocios")||[]).map(n=>n.nome||"").filter(Boolean)])].filter(Boolean);return lst.map(n=>`<option value="${esc(n)}" ${n===atual?"selected":""}>${esc(n)}</option>`).join("")||`<option value="">—</option>`})()}</select>
<label style="font-size:12px;color:#64748b;">Situação</label>
<select id="gt-ef-status" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:8px;margin:4px 0 16px;background:#fff;color:#0f172a;">
...
<button id="gt-ef-excluir" style="padding:9px 14px;border:1px solid #fca5a5;color:#b91c1c;background:#fff;border-radius:8px;cursor:pointer;">Excluir</button>
<div style="display:flex;gap:8px;">
<button id="gt-ef-cancelar" style="padding:9px 14px;border:1px solid #e2e8f0;background:#fff;color:#0f172a;border-radius:8px;cursor:pointer;">Cancelar</button>
<button id="gt-ef-salvar" style="padding:9px 16px;border:none;background:#2563eb;color:#fff;border-radius:8px;cursor:pointer;font-weight:600;">Salvar</button>
</div></div></div>`;document.body.appendChild(ov);let df=false;ov.addEventListener("mousedown",ev=>{df=(ev.target===ov)});ov.addEventListener("click",ev=>{if(ev.target===ov&&df)ov.remove();df=false});ov.querySelector("#gt-ef-cancelar").addEventListener("click",()=>ov.remove());ov.querySelector("#gt-ef-salvar").addEventListener("click",async()=>{const venc=ov.querySelector("#gt-ef-venc").value;const nd={...d,descricao:ov.querySelector("#gt-ef-desc").value,valor:Number(ov.querySelector("#gt-ef-valor").value)||0,status:ov.querySelector("#gt-ef-status").value,negocio:ov.querySelector("#gt-ef-neg").value};if(venc){nd.data_vencimento=venc;nd.mes_ref=venc.slice(0,7)}try{const{error:e2}=await g.from("lancamentos").update({dados:nd}).eq("id",id);if(e2)throw e2;c.ok("Fatura atualizada!");ov.remove();await w()}catch(e2){c.err("Erro ao salvar: "+e2.message)}});ov.querySelector("#gt-ef-excluir").addEventListener("click",async()=>{if(!confirm("Excluir esta fatura? Essa ação não tem volta."))return;try{const{error:e2}=await g.from("lancamentos").delete().eq("id",id);if(e2)throw e2;c.ok("Fatura excluída.");ov.remove();await w()}catch(e2){c.err("Erro ao excluir: "+e2.message)}})}catch(e2){c.err("Erro: "+e2.message)}}
async function j(e){if(!confirm("Marcar esta fatura como paga?"))return;const a=new Date().toISOString().split("T")[0];try{const{data:o}=await g.from("lancamentos").select("dados").eq("id",e).single(),i={...(o==null?void 0:o.dados)||{},status:"Pago",dataPagamento:a},{error:r}=await g.from("lancamentos").update({dados:i}).eq("id",e);if(r)throw r;c.ok("Marcado como pago."),await w()}catch(o){c.err("Erro: "+o.message)}}async function I(e){if(!e)return;const o=(u.get("clientes")||[]).find(r=>r.id===e);if(!o){c.err("Cliente não encontrado.");return}const i=(o.contratos||[]).map(r=>({...r,bloqueado:!0}));try{const{error:r}=await g.from("cli_clientes").update({dados:{...o,contratos:i}}).eq("id",e);if(r)throw r;c.ok("Cliente bloqueado."),await w()}catch(r){c.err("Erro: "+r.message)}}function C(e){return String(e||"").replace(/</g,"&lt;")}async function B(e){d=e,m="",v="",x="todos",p=new Set,d.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:200px;color:#9ca3af;">Carregando…</div>',await w()}export{B as initCobrancas};
