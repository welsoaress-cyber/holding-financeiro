import{s as g,f as M,a as A,u as O,t as E,N as Y,b as H,h as j,c as U,d as V,e as G,g as Q}from"./page-dashboard-Dbqm2OjXbF1.js";let a=null,D=null;function J(){var e;return a||(a=document.createElement("div"),a.id="modal-overlay",a.setAttribute("aria-modal","true"),a.setAttribute("role","dialog"),Object.assign(a.style,{position:"fixed",inset:"0",background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:"1000",padding:"16px",overflowY:"auto",opacity:"0",transition:"opacity .18s ease"}),a.innerHTML=`
  ...
  `}async function R(e){var d;f.abrir({titulo:e?"Editar lançamento":"Novo lançamento",conteudo:te(e)}),await new Promise(s=>setTimeout(s,60));const t=document.getElementById("form-lanc");if(!t)return;const o=t.querySelector("#check-recorrente"),n=t.querySelector("#div-recorrencia");o==null||o.addEventListener("change",()=>{n&&(n.style.display=o.checked?"flex":"none")}),t.addEventListener("submit",async s=>{s.preventDefault();const i=new FormData(t),p=i.get("data_vencimento")||j(),m=i.get("recorrente")==="on",w=m?parseInt(i.get("qtd_meses")||"1",10):0,u={...(e??{}),id:(e==null?void 0:e.id)??B(),negocio:i.get("negocio")||null,tipo:i.get("tipo"),status:i.get("status"),cliente:i.get("cliente")||null,descricao:i.get("descricao"),valor:parseFloat(i.get("valor"))||0,data_vencimento:p,data_pagamento:i.get("data_pagamento")||null,categoria:i.get("categoria")||null,mes_ref:p.slice(0,7)},y=t.querySelector("[type=submit]");y.disabled=!0,y.textContent="Salvando…";try{if(m&&w>0){const x=[u];for(let r=1;r<=w;r++){const b=U(p,r);x.push({...u,id:B(),data_vencimento:b,mes_ref:b.slice(0,7)})}await V("lancamentos",x),x.filter(r=>r.mes_ref===F()).forEach(r=>g.patch("lancamentos",r.id,r)),E.ok(`${x.length} lançamentos criados!`)}else await G("lancamentos",u);(!e||u.mes_ref===F())&&g.patch("lancamentos",u.id,u);const _cliCh=e&&(e.cliente||null)!==(u.cliente||null);if(_cliCh){const _esc=await f.escopoEdicao(e.cliente||"(sem nome)",u.cliente||"(sem nome)");if(_esc==="proximos"||_esc==="todos"){const _alvo=(g.get("lancamentos")??[]).filter(_r=>_r.id!==u.id&&(_r.cliente||null)===(e.cliente||null)&&(_esc==="todos"||(_r.status!=="pago"&&(_r.data_vencimento||_r.data||"")>=u.data_vencimento)));for(const _r of _alvo){try{await G("lancamentos",{..._r,cliente:u.cliente});g.patch("lancamentos",_r.id,{..._r,cliente:u.cliente})}catch(_ex){console.error(_ex)}}E.ok(`Nome atualizado em ${_alvo.length+1} lançamento(s)!`)}else{E.ok("Lançamento atualizado!")}}else{E.ok(e?"Lançamento atualizado!":"Lançamento criado!")};f.fechar(),h()}catch(x){console.error(x),E.err("Erro ao salvar: "+x.message),y.disabled=!1,y.textContent=e?"Salvar":"Criar"}}),(d=document.getElementById("btn-excluir"))==null||d.addEventListener("click",async()=>{if(await f.confirmar(`Excluir "${e.descricao}"? Esta ação não pode ser desfeita.`,{textoBotao:"Excluir",tipo:"danger"}))try{await Q("lancamentos",e.id),g.remove("lancamentos",e.id),f.fechar(),E.ok("Lançamento excluído."),h()}catch(i){E.err("Erro ao excluir: "+i.message)}})}async function oe(e){l=e;const t=new Date;v=t.getFullYear(),c=t.getMonth()+1,L=null,q=!1,$="todos",k="todos",z="",nb="todos",T=!1,g.set("lancamentos",[]),h(),await _(!0)}const re=Object.freeze(Object.defineProperty({__proto__:null,initLancamentos:oe},Symbol.toStringTag,{value:"Module"}));export{re as i,f as m,B as u};
