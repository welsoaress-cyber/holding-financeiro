import{s as g,f as M,a as A,u as O,t as E,N as Y,b as H,h as j,c as U,d as V,e as G,g as Q,i as _fa}from"./page-dashboard-Dbqm2OjXb.js";let a=null,D=null;function J(){var e;return a||(a=document.createElement("div"),a.id="modal-overlay",a.setAttribute("aria-modal","true"),a.setAttribute("role","dialog"),Object.assign(a.style,{position:"fixed",inset:"0",background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:"1000",padding:"16px",overflowY:"auto",opacity:"0",transition:"opacity .18s ease"}),a.innerHTML=`
    ...
    <form id="form-lanc" style="display:flex;flex-direction:column;gap:14px;">
      ...
      <label style="display:flex;flex-direction:column;gap:4px;font-size:13px;font-weight:500;">
        ...
        <div style="position:relative;">
          <input id="inp-cliente" name="cliente" type="text" value="${o.cliente||""}" autocomplete="off"
            placeholder="Nome do cliente (opcional)"
            style="padding:8px;border-radius:8px;border:1px solid var(--border,#e5e7eb);font-size:14px;background:var(--bg-card);color:var(--text);width:100%;box-sizing:border-box;" />
          <div id="ac-clientes" style="display:none;position:absolute;top:100%;left:0;right:0;background:var(--bg-card,#fff);border:1px solid var(--border,#e5e7eb);border-radius:8px;margin-top:2px;max-height:180px;overflow-y:auto;z-index:9999;box-shadow:0 4px 16px rgba(0,0,0,.12);"></div>
        </div>
      </label>
      ...
      <label style="display:flex;flex-direction:column;gap:4px;font-size:13px;font-weight:500;">
        ...
        <datalist id="dl-cat-lanc">
          ${[...new Set([...(g.get("categorias")||[]).map(_ct=>_ct.nome||""),...(g.get("lancamentos")||[]).map(l=>l.categoria||"")])].filter(Boolean).sort().map(n=>`<option value="${n}">`).join("")}
        </datalist>
      </label>
  ...
  `}async function R(e){var d;await Promise.all([g.get("negocios")?.length?null:A("negocios","*",{limit:200}).then(({items:_nb})=>{if(_nb.length)g.set("negocios",O("negocios",_nb))}).catch(_ex=>console.error("[negocios]",_ex)),g.get("clientes")?.length?null:A("cli_clientes","*",{limit:500}).then(({items:_cl})=>{if(_cl.length)g.set("clientes",O("cli_clientes",_cl))}).catch(_ex=>console.error("[clientes]",_ex)),g.get("contas")?.length?null:A("contas","*",{limit:200}).then(({items:_ct})=>{if(_ct.length)g.set("contas",_ct)}).catch(_ex=>console.error("[contas]",_ex)),g.get("categorias")?.length?null:A("categorias","id,dados",{limit:500}).then(({items:_ctg})=>{if(_ctg.length)g.set("categorias",O("categorias",_ctg))}).catch(_ex=>console.error("[categorias]",_ex))].filter(Boolean));f.abrir({titulo:e?"Editar lançamento":"Novo lançamento",conteudo:te(e)}),await new Promise(s=>setTimeout(s,60));const t=document.getElementById("form-lanc");if(!t)return;const _inpClt=t.querySelector("#inp-cliente"),_acClt=t.querySelector("#ac-clientes");if(_inpClt&&_acClt){_inpClt.addEventListener("input",()=>{const _q=_inpClt.value.trim().toLowerCase();if(!_q){_acClt.style.display="none";_acClt.innerHTML="";return}const _ms=(g.get("clientes")||[]).filter(_c=>(_c.nome||"").toLowerCase().includes(_q)).slice(0,10);if(!_ms.length){_acClt.style.display="none";_acClt.innerHTML="";return}_acClt.innerHTML=_ms.map(_c=>`<div data-nome="${_c.nome}" style="padding:8px 12px;cursor:pointer;font-size:14px;color:var(--text,#111);border-bottom:1px solid var(--border,#e5e7eb);">${_c.nome||""}</div>`).join("");_acClt.style.display="block";_acClt.querySelectorAll("[data-nome]").forEach(_it=>{_it.addEventListener("mouseover",()=>{_it.style.background="var(--bg,#f8fafc)"});_it.addEventListener("mouseout",()=>{_it.style.background=""});_it.addEventListener("mousedown",_ev=>{_ev.preventDefault();_inpClt.value=_it.dataset.nome;_acClt.style.display="none";_acClt.innerHTML=""})})});_inpClt.addEventListener("blur",()=>setTimeout(()=>{_acClt.style.display="none"},150))}const o=t.querySelector("#check-recorrente"),n=t.querySelector("#div-recorrencia");o==null||o.addEventListener("change",()=>{n&&(n.style.display=o.checked?"flex":"none")});const _st2=t.querySelector("[name=status]"),_dp2=t.querySelector("[name=data_pagamento]"),_ldp2=t.querySelector("#label-data-pgto");if(_st2&&_dp2){const _upd2=()=>{const _isPagoOuParcial=_st2.value==="pago"||_st2.value==="parcial";if(_ldp2)_ldp2.style.display=_isPagoOuParcial?"flex":"none";if(_isPagoOuParcial&&!_dp2.value)_dp2.value=j()};_st2.addEventListener("change",_upd2);_upd2()}const _selNeg=t.querySelector("#sel-negocio"),_inpNovNeg=t.querySelector("#inp-negocio-novo");if(_selNeg&&_inpNovNeg){_selNeg.addEventListener("change",()=>{const _isNovo=_selNeg.value==="__novo__";_inpNovNeg.style.display=_isNovo?"":"none";if(_isNovo)_inpNovNeg.focus()})}const _selFreq=t.querySelector("#sel-frequencia"),_divRecorr2=t.querySelector("#div-recorrencia");const _selFimTipo=t.querySelector("#sel-fim-tipo"),_divFimVezes=t.querySelector("#div-fim-vezes"),_divFimData=t.querySelector("#div-fim-data");const _updFreq=()=>{const _isR=_selFreq?.value!=="unica";if(_divRecorr2)_divRecorr2.style.display=_isR?"flex":"none"};const _updFim=()=>{const _ft=_selFimTipo?.value;if(_divFimVezes)_divFimVezes.style.display=_ft!=="data"&&_ft!=="fixa"?"":"none";if(_divFimData)_divFimData.style.display=_ft==="data"?"":"none"};if(_selFreq){_selFreq.addEventListener("change",_updFreq);_updFreq()}if(_selFimTipo){_selFimTipo.addEventListener("change",_updFim);_updFim()}t.addEventListener("submit",async s=>{s.preventDefault();const i=new FormData(t),p=i.get("data_vencimento")||j(),_freq=i.get("frequencia")||"unica",_recorre=_freq!=="unica",_fimTipo=i.get("fim_tipo")||"vezes",_fimVezes=parseInt(i.get("fim_vezes")||"12",10),_fimData=i.get("fim_data")||"",m=_recorre,w=0,u={...(e??{}),id:(e==null?void 0:e.id)??B(),negocio:(()=>{const _snv=t.querySelector("#sel-negocio")?.value||"",_inv=t.querySelector("#inp-negocio-novo")?.value||"";return _snv==="__novo__"?(_inv.trim()||null):(_snv||null)})(),tipo:i.get("tipo"),status:i.get("status"),cliente:i.get("cliente")||null,descricao:i.get("descricao"),valor:parseFloat(i.get("valor"))||0,data_vencimento:p,data_pagamento:i.get("data_pagamento")||null,categoria:i.get("categoria")||null,conta:i.get("conta")||null,forma_pagamento:i.get("forma_pagamento")||null,frequencia:_freq,fim_tipo:_fimTipo,fim_vezes:_fimVezes,fim_data:_fimData||null,mes_ref:p.slice(0,7)},y=t.querySelector("[type=submit]");y.disabled=!0,y.textContent="Salvando…";try{const _selN2=t.querySelector("#sel-negocio"),_inpN2=t.querySelector("#inp-negocio-novo");if(_selN2?.value==="__novo__"&&_inpN2?.value?.trim()){const _nNeg={id:B(),nome:_inpN2.value.trim()};try{await G("negocios",_nNeg);g.patch("negocios",_nNeg.id,_nNeg)}catch(_ex){console.error("Erro ao criar negócio:",_ex)}}if((u.status==="pago"||u.status==="parcial")&&!u.data_pagamento)u.data_pagamento=j();if(_recorre&&!e){const _apd=(dt,fr,n)=>{const _d=new Date(dt+"T12:00:00");if(fr==="diaria")_d.setDate(_d.getDate()+n);else if(fr==="semanal")_d.setDate(_d.getDate()+7*n);else if(fr==="quinzenal")_d.setDate(_d.getDate()+15*n);else if(fr==="mensal")_d.setMonth(_d.getMonth()+n);else if(fr==="bimestral")_d.setMonth(_d.getMonth()+2*n);else if(fr==="trimestral")_d.setMonth(_d.getMonth()+3*n);else if(fr==="semestral")_d.setMonth(_d.getMonth()+6*n);else if(fr==="anual")_d.setFullYear(_d.getFullYear()+n);return _d.toISOString().slice(0,10)};const x=[u];if(_fimTipo==="vezes"){for(let r=1;r<_fimVezes;r++){const b=_apd(p,_freq,r);x.push({...u,id:B(),data_vencimento:b,data_pagamento:null,status:"pendente",mes_ref:b.slice(0,7)})}}else if(_fimTipo==="data"&&_fimData){let r=1;while(true){const b=_apd(p,_freq,r++);if(b>_fimData||r>600)break;x.push({...u,id:B(),data_vencimento:b,data_pagamento:null,status:"pendente",mes_ref:b.slice(0,7)})}}else{for(let r=1;r<=60;r++){const b=_apd(p,_freq,r);x.push({...u,id:B(),data_vencimento:b,data_pagamento:null,status:"pendente",mes_ref:b.slice(0,7)})}}await V("lancamentos",x),x.filter(r=>r.mes_ref===F()).forEach(r=>g.patch("lancamentos",r.id,r)),E.ok(`${x.length} lançamento(s) criado(s)!`)}else await G("lancamentos",u);(!e||u.mes_ref===F())&&g.patch("lancamentos",u.id,u);if(e){const _esc=await f.escopoAcao("Aplicar alterações",e,"primary");if(_esc==="proximos"||_esc==="todos"){const _allLanc=O("lancamentos",await _fa("lancamentos","id,dados"));const _alvo=_allLanc.filter(_r=>_r.id!==u.id&&(_r.descricao||"")===(e.descricao||"")&&(_r.cliente||null)===(e.cliente||null)&&(_esc==="todos"||(_r.status!=="pago"&&(_r.data_vencimento||_r.data||"")>=u.data_vencimento)));const _campos={cliente:u.cliente,descricao:u.descricao,categoria:u.categoria,negocio:u.negocio,valor:u.valor,tipo:u.tipo,conta:u.conta,forma_pagamento:u.forma_pagamento};for(const _r of _alvo){try{await G("lancamentos",{..._r,..._campos});if((_r.mes_ref||"")===F())g.patch("lancamentos",_r.id,{..._r,..._campos})}catch(_ex){console.error(_ex)}}let _criados=0;if(_recorre&&_esc==="proximos"){const _apd2=(dt,fr,n)=>{const _d=new Date(dt+"T12:00:00");if(fr==="diaria")_d.setDate(_d.getDate()+n);else if(fr==="semanal")_d.setDate(_d.getDate()+7*n);else if(fr==="quinzenal")_d.setDate(_d.getDate()+15*n);else if(fr==="mensal")_d.setMonth(_d.getMonth()+n);else if(fr==="bimestral")_d.setMonth(_d.getMonth()+2*n);else if(fr==="trimestral")_d.setMonth(_d.getMonth()+3*n);else if(fr==="semestral")_d.setMonth(_d.getMonth()+6*n);else if(fr==="anual")_d.setFullYear(_d.getFullYear()+n);return _d.toISOString().slice(0,10)};const _existMesRef=new Set(_allLanc.filter(_r=>(_r.descricao||"")===(u.descricao||"")&&(_r.cliente||null)===(u.cliente||null)).map(_r=>_r.mes_ref));const _novos=[];const _limN=_fimTipo==="vezes"?_fimVezes-1:60;for(let _ri=1;_ri<=_limN;_ri++){const _bd=_apd2(p,_freq,_ri);if(_fimData&&_bd>_fimData)break;if(!_existMesRef.has(_bd.slice(0,7)))_novos.push({...u,id:B(),data_vencimento:_bd,data_pagamento:null,status:"pendente",mes_ref:_bd.slice(0,7)})};if(_novos.length){await V("lancamentos",_novos);_novos.filter(_r=>_r.mes_ref===F()).forEach(_r=>g.patch("lancamentos",_r.id,_r));_criados=_novos.length}}E.ok(`Alteração aplicada em ${_alvo.length+1} lançamento(s)!`+(_criados?` ${_criados} mês(es) criado(s).`:""))}else if(_esc==="este"){E.ok("Lançamento atualizado!")}else{E.ok("Lançamento atualizado!")}}else{E.ok("Lançamento criado!")};f.fechar(),h()}catch(x){console.error(x),E.err("Erro ao salvar: "+x.message),y.disabled=!1,y.textContent=e?"Salvar":"Criar"}}),(d=document.getElementById("btn-excluir"))==null||d.addEventListener("click",async()=>{{const _escDel=await f.escopoAcao("Excluir lançamento",e,"danger");if(_escDel!==null){try{const _allLanc=O("lancamentos",await _fa("lancamentos","id,dados"));const _alvoDel=_allLanc.filter(_rd=>_rd.id!==e.id&&(_rd.descricao||"")===(e.descricao||"")&&(_rd.cliente||null)===(e.cliente||null)&&(_escDel==="todos"||(_rd.status!=="pago"&&(_rd.data_vencimento||_rd.data||"")>=(e.data_vencimento||e.data||""))));await Q("lancamentos",e.id);g.remove("lancamentos",e.id);for(const _rd of _alvoDel){try{await Q("lancamentos",_rd.id);g.remove("lancamentos",_rd.id)}catch(_ex){console.error(_ex)}}f.fechar();E.ok(_alvoDel.length?`${_alvoDel.length+1} lançamentos excluídos.`:"Lançamento excluído.");h()}catch(i){E.err("Erro ao excluir: "+i.message)}}}})}async function oe(e){l=e;const t=new Date;v=t.getFullYear(),c=t.getMonth()+1,L=null,q=!1,$="todos",k="todos",z="",nb="todos",sel=new Set(),T=!1,g.set("lancamentos",[]),h(),await _(!0);Promise.all([A("negocios","*",{limit:200}).then(({items:_nb})=>{if(_nb.length&&!g.get("negocios")?.length)g.set("negocios",O("negocios",_nb))}).catch(()=>{}),A("cli_clientes","*",{limit:500}).then(({items:_cl})=>{if(_cl.length&&!g.get("clientes")?.length)g.set("clientes",O("cli_clientes",_cl))}).catch(()=>{}),A("contas","*",{limit:200}).then(({items:_ct})=>{if(_ct.length&&!g.get("contas")?.length)g.set("contas",_ct)}).catch(()=>{}),A("categorias","id,dados",{limit:500}).then(({items:_ctg})=>{if(_ctg.length&&!g.get("categorias")?.length)g.set("categorias",O("categorias",_ctg))}).catch(()=>{})]).catch(()=>{})}const re=Object.freeze(Object.defineProperty({__proto__:null,initLancamentos:oe},Symbol.toStringTag,{value:"Module"}));export{re as i,f as m,B as u};
