...
2. **📄 Documentos → Google Drive** — organizados automaticamente por **tipo e ano**:
   `Apps Tom/Planilhas/2023/orcamento.xlsx`, `Apps Tom/PDFs/2021/contrato.pdf`…
...
### Android
1. Abra a pasta `apps-tom/` no **Android Studio** e rode o app (`composeApp`)
2. Ou: `./gradlew :composeApp:assembleDebug`

### iOS (precisa de um Mac com Xcode)
1. `brew install xcodegen`
2. `cd iosApp && xcodegen generate`
3. Abra `AppsTom.xcodeproj` no Xcode e rode — o Xcode compila o código
...
1. Acesse [console.cloud.google.com](https://console.cloud.google.com) e crie um projeto (ex.: "Apps Tom")
...
4. Em **Credenciais → Criar credenciais → ID do cliente OAuth**:
   - **Android**: pacote `br.com.appstom` + SHA-1 (`./gradlew signingReport`, build `debug`)
   - **iOS**: bundle id `br.com.appstom.AppsTom` (necessário quando for integrar o GIDSignIn)
